...
В-третьих, общение между реверс-прокси и сайтом приложений идёт по `HTTP`, дабы упростить конфигурацию сервера приложений и не тратить время и силы на обмен HTTPS-сертификатами и ключами.  
Увы, по умолчанию, **`WordPress`** постоянно пытается перейти на `HTTPS`-протокол. Для вразумления пациента в файл **wp-config.php** было добавлено следующее:  
```php
