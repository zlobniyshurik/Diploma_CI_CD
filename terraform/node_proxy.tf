resource "yandex_compute_instance" "proxy" {
  ...
  # дистрибутива без проблем выкачать.
  resources {
    cores  = 2
  ...
  #
  # UPD. На самом деле, нет...Через Ansible сильно проще.
  # Оставляю этот кусок кода, как напоминание себе о том,
  # почему из TF лучше не менять содержимое VM - получается сложно, объёмно,
  # и практически непортируемо на другие конфигурации VM.
  connection {
    type     = "ssh"
  ...
  provisioner "file" {
    source = "autogenerated/nginx.conf"
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo mv /tmp/config /home/${var.login_name}/.ssh",
      "sudo chown ${var.login_name}:${var.login_name} /home/${var.login_name}/.ssh/config",
      "sudo chmod 400 /home/${var.login_name}/.ssh/config",
